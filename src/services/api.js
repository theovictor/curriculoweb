const BASE_URL = process.env.REACT_APP_API_BASEURL;

// const BASE_URL = 'http://localhost:3333/';

export const api_login = `${BASE_URL}/login`
export const api_cadastro = `${BASE_URL}/cadastro`
export const api_user_att = `${BASE_URL}/update`
export const api_curriculo = `${BASE_URL}/curriculo`
export const api_formacao = `${BASE_URL}/curriculo/formacao`
export const api_conhecimento = `${BASE_URL}/curriculo/conhecimento`
export const api_experiencia = `${BASE_URL}/curriculo/experiencia`
export const api_file = `${BASE_URL}/file`
export const api_file2 = `${BASE_URL}/update`
export const api_user = `${BASE_URL}/user`
